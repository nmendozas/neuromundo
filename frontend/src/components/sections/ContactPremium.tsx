...
export function ContactPremium({ parameters }: { parameters: Record<string, string> }) {
  ...
  const address = [parameters['company.address'], parameters['company.city']].filter(Boolean).join(' · ') || 'Colombia';
  ...
  const whatsapp = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hola ${companyName} 👋 Quisiera conversar sobre una solución para mi institución.`)}`;
